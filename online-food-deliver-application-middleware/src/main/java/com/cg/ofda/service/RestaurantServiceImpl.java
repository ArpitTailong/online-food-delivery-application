...
public class RestaurantServiceImpl implements IRestaurantService {
	...
	@Override
	public List<RestaurantModel> viewAllRestaurantByItem(String itemName) throws RestaurantException, ItemException {
		
		ItemEntity item= itemRepo.findByItemName(itemName);
		
		if(item==null) {
			throw new ItemException("no item with @"+ itemName+ " found");
		}
		
		List<RestaurantEntity> rest= item.getRestaurants();
		
		return rest.stream().map(parser::parse).collect(Collectors.toList());
	}
