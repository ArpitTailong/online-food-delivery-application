...
public class EMParserLogin {
	...
	public LoginModel parse(LoginEntity source) {
		return source == null ? null : new LoginModel(source.getUserid(), source.getUserName(), source.getPassword(),source.getCustomer().getCustomerId());
	...
	public LoginEntity parse(LoginModel source) {
		return source == null ? null : new LoginEntity(source.getUserid(), source.getUserName(), source.getPassword(),custRepo.findById(source.getCustomerId()).orElse(null));
	}
}